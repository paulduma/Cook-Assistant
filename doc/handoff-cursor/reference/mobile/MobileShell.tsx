// reference/mobile/MobileShell.tsx
// Coque mobile : barre supérieure (titre « Chez Verdi » conservé ou titre d'écran
// + retour) et barre d'onglets inférieure (Recettes / Planning / Liste).
// Safe-areas iOS gérées via env(safe-area-inset-*).
//
// Branchez les onglets et le retour sur votre router.

import React from 'react';
import { Icon, IconName } from '../Icon';

type TabKey = 'recettes' | 'planning' | 'courses';
...
export function MobileTopBar({
  ...
  title?: string;
  back?: boolean;
  wordmark?: boolean; // affiche « Chez Verdi » au lieu d'un titre d'écran
  onBack?: () => void;
}) {
  return (
    <div
      className="flex items-center gap-3 px-[18px] pb-3 bg-cream border-b border-line"
...
export function MobileTabBar({
  ...
  return (
    <nav
      className="bg-cream border-t border-line flex px-4 pt-2.5"
...
/* Conteneur d'écran mobile : colonne pleine hauteur, zone centrale scrollable. */
