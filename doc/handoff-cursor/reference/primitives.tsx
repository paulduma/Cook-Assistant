// reference/primitives.tsx
// Briques visuelles partagées. Tailwind only — aucune logique métier.
// Réutilisez-les dans vos écrans pour garder la cohérence éditoriale.

import React from 'react';
import { Icon, IconName } from './Icon';

/* ── Kicker : sur-titre Archivo uppercase, accent ember ───────────── */
...
/* ── Bouton : variantes solid / outline / danger / ghost ──────────── */
...
/* ── Filet de section : titre + trait qui le prolonge ─────────────── */
...
/* ── Champ « souligné » (pas de boîte) pour les formulaires ───────── */
export function Field({
  ...
  value?: string;
  placeholder?: string;
  onChange?: (v: string) => void; // ⟵ BRANCHER votre state d'édition
}) {
  return (
...
/* ── Vignette image : placeholder rayé tant qu'il n'y a pas de photo ─
   Remplacez par <img src={recipe.image_url} .../> quand l'URL existe. */
export function Thumb({
  ...
  label?: string;
  className?: string;
  src?: string | null; // ⟵ BRANCHER recipe.image_url
}) {
  if (src) {
